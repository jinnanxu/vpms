<template>
  <div>
    ...
    <el-table
      ...
      style="width: 100%"
    >
      <el-table-column align="center" prop="publishDate" label="发布时间" show-overflow-tooltip>
      ...
      <el-table-column align="center" prop="teacher.realName" label="作者" show-overflow-tooltip>
        <template slot-scope="scope">
          <span class="cspan">{{ scope.row.expert.workDept }}</span>
          <span class="tspan">{{ scope.row.expert.realName }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="tags" label="关键词" width="120">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    handleEdit: function (row) {
      ...
      if (row!=null) {
        console.log('编辑……')
        this.editForm.baseId = row.baseId;
    ...
    offShelf(row){
      ...
      this.$confirm(msg, "提示", {
        ...
        AdminDeleteKnowledge({kid: row.baseId})
          .then((res) => {
            if(res.success){
              this.$message({
                type: "success",
