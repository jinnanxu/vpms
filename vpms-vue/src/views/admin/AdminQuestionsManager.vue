    ...
    >
      ...
      <el-table-column align="center" prop="role" label="状态" width="100">
        <template slot-scope="scope">
          ...
          <el-tag v-else-if="scope.row.status == 2" type="success" size="small">已解决</el-tag>
        </template>
      </el-table-column>
...
<script>
import { AdminGetQuestionList,AdminDeleteQuestion,TeacherSaveAnswer } from '@/api/api.js'
  ...
  methods: {
    ...
    offShelf(row){
      ...
      this.$confirm(msg, "提示", {
        ...
        AdminDeleteQuestion({qid: row.questionId})
          .then((res) => {
            if(res.success){
              this.$message({
                type: "success",
