<template>
  <div>
    ...
    <el-table
      ...
      style="width: 100%"
    >
      <el-table-column align="center" prop="publishDate" label="发布时间" width="150">
        <template slot-scope="scope">
          {{ scope.row.publishDate|timestampToDayHM }}
        </template>
      </el-table-column>
      <el-table-column align="center" prop="title" label="标题" show-overflow-tooltip>
      </el-table-column>
      <el-table-column align="center" prop="tags" label="关键词">
        <template slot-scope="scope">
          <el-tag type="warning" size="small" v-for="(itemChild,indexChild) in scope.row.tags.split(';')" :key="indexChild">{{ itemChild }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="expert.realName" label="作者" show-overflow-tooltip>
        <template slot-scope="scope">
          <span>{{ scope.row.expert.workDept }}</span>
          <span class="tspan">{{ scope.row.expert.realName }}</span>
        </template>
      </el-table-column>
      <!-- <el-table-column align="center" prop="viewCount" label="搜索/浏览量" width="120">
      </el-table-column> -->
      <el-table-column align="center" label="操作" width="300">
        <template slot-scope="scope">
          <el-button size="mini" type="success" @click="handleDetail(scope.row)" icon="el-icon-document">详情</el-button>
          <el-button
            v-if="scope.row.expert.adminId == currExpert.adminId"
            size="mini"
            type="primary"
            icon="el-icon-edit"
            ...
            >修改</el-button
          >
          <el-button size="mini" type="danger" @click="offShelf(scope.row, -1)" v-if="scope.row.expert.adminId == currExpert.adminId" icon="el-icon-delete">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
      ...
      >
        ...
        <el-form-item label="关键字" prop="tags">
          <el-input
            ...
            auto-complete="off"
            placeholder="如：水肥管理;虫害防治"
          ></el-input>
        </el-form-item>
        <el-form-item label="内容正文" prop="detail" id='quillEditorQiniu'>
          <quill-editor class="editor" v-model="editForm.detail" ref="customQuillEditor" :options="editorOption" >
          </quill-editor>
    ...
    <div v-html="currDetailData.detail" class="introduction"></div>
    </el-dialog>
  </div>
</template>

<script>
import { AdminUpdateKnowledge,AdminGetKnowledgeList,AdminDeleteKnowledge } from '@/api/api.js'
...
export default {
  ...
  methods: {
    ...
    getdata(parameter) {
      this.loading = true
      AdminGetKnowledgeList(parameter)
        .then((res) => {
          this.loading = false;
          if (res.success) {
    ...
    handleEdit: function (row) {
      ...
      if (row!=null) {
        console.log('编辑……')
        this.editForm.knowledgeId = row.knowledgeId;
        this.editForm.title = row.title;
        this.editForm.tags = row.tags
        this.editForm.status = row.status
        this.editForm.detail = row.detail
        this.editForm.viewCount = row.viewCount
      } else {
        console.log('新增……')
        this.editForm.knowledgeId = null
        this.editForm.title = ""
        this.editForm.status = 1
        this.editForm.tags = ""
        this.editForm.detail = ""
        this.editForm.viewCount = 0
      }
    },
    // 编辑、增加页面保存方法
    submitForm(editData) {
      this.$refs[editData].validate((valid) => {
        ...
        // 调用保存接口
        AdminUpdateKnowledge(this.editForm)
          .then((res) => {
    ...
    offShelf(row){
      ...
      this.$confirm(msg, "提示", {
        ...
        AdminDeleteKnowledge({kid: row.knowledgeId})
          .then((res) => {
            if(res.success){
              this.$message({
                type: "success",
